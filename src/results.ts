...
function parseTestcases(xml: string): { cases: number; failed: number; skipped: number; failures: TestFailure[] } {
  let cases = 0
  let failed = 0
  let skipped = 0
...
export function reportDirectory(projectRoot: string, modulePath: string, taskName: string): string {
...
export function collectReportFiles(projectRoot: string, modulePath: string, taskName: string): string[] {
  const files: string[] = []
  walkReportFiles(reportDirectory(projectRoot, modulePath, taskName), files, 0)
  return files.sort()
}

export function parseReports(files: string[], minimumMtimeMs?: number): ReportParseResult {
  let total = 0
  let failed = 0
  let skipped = 0
  const failures: TestFailure[] = []
